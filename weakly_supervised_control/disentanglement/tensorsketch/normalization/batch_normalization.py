...
from weakly_supervised_control.disentanglement.tensorsketch.normalization.base import Norm
from weakly_supervised_control.disentanglement.tensorsketch.modules.base import build_with_name_scope
from weakly_supervised_control.disentanglement.tensorsketch.utils import assign_moving_average
    ...
    def build_parameters(self, x):
        ...
        if self.track_statistics:
            self.running_mean = tf.Variable(
                tf.zeros([num_dims]), trainable=False)
            self.running_variance = tf.Variable(
                tf.ones([num_dims]), trainable=False)
        ...
        if training:
            if fuse:
                ...
                mean, variance = tf.nn.moments(
                    x, list(range(len(x.shape) - 1)))
                x = tf.nn.batch_normalization(
                    x, mean, variance, bias, scale, epsilon)
