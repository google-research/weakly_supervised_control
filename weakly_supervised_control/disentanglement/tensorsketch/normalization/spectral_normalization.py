...
from weakly_supervised_control.disentanglement.tensorsketch.normalization.base import KernelNorm
...
class SpectralNorm(KernelNorm):
    ...
    def build_parameters(self, kernel):
        num_input, num_output = tf.reshape(
            kernel, (-1, kernel.shape[-1])).shape
        self.u = tf.Variable(
